...
public class AzureCloudConfigRefresh implements ApplicationEventPublisherAware {
	...
	/**
	 * Checks configurations to see if they are no longer cached. If they are no
	 * longer cached they are updated.
	 * 
	 * @return Future with a boolean of if a RefreshEvent was published. If
	 *         refreshConfigurations is currently being run elsewhere this method
	 *         will return right away as <b>false</b>.
	 */
	...
	/**
	 * Goes through each config store and checks if any of its keys need to be
	 * refreshed. If any store has a value that needs to be updated a refresh event
	 * is called after every store is checked.
	 * 
	...
	/**
	 * Checks un-cached items for etag changes. If they have changed a
	 * RefreshEventData is published.
	 * 
	 * @param store
	 *            the {@code store} for which to composite watched key names
	 * @param storeSuffix
	 *            Suffix used to distinguish between Settings and Features
	 * @param watchedKeyNames
	 *            Key used to check if refresh should occur
	...
	/**
	 * For each refresh, multiple etags can change, but even one etag is changed,
	 * refresh is required.
	 */
