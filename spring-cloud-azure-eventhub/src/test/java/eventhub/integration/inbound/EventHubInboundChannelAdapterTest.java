...
public class EventHubInboundChannelAdapterTest {
    ...
    @Mock
    private EventHubSubscriber eventHubSubscriber;
    @Mock
    ...
    public void setUp() {
        ...
        when(eventHubOperation.subscribe(eq(eventHubName), eq(consumerGroup))).thenReturn(eventHubSubscriber);
        when(eventHubSubscriber.getCheckpointer()).thenReturn(checkpointer);
    }
