...
The binder also supports link:reference-doc#spring-cloud-azure-resourcemanager[Spring Could Azure Resource Manager] by default. To learn about how to retrieve the connection string with security principals that are not granted with `Data` related roles, please refer to the link:reference-doc#resource-manager-basic-usage[resource manager example] for details.
