...
When you build a web application, identity and access management are two foundational pieces.

Azure offers a great platform to democratize your application developing journey, as it not only offers a cloud-based identity service, but also provides a deep integration with the rest of the Azure ecosystem.

`Spring Security` makes it easy to secure your Spring based application with powerful abstractions and extensible interfaces. However, as powerful as `Spring framework`, `Spring Security` is not tailored to a specific identity provider, like `Azure AD`.

The `spring-cloud-azure-starter-active-directory` (`aad-starter` for short) provides a most optimal way to connect your `web application` to an Azure Active Directory (Azure AD) tenant and protects a `resource server` with Azure AD. It uses the `OAuth 2.0` protocol to protect a `web application` or a `resource server`.
...
Now you start the application and access your application by opening a browser, then you will be redirected to the Microsoft login page.
...
In previous sample, all scopes will be consented when customer first login, no matter it belongs to `resource-server-1` or `resource-server-2`. If you don't want the customer consent all scopes at the first login, you can do like this:
...
This scenario doesn't support login, it just protects the server by validating the `access token`. If the `access token` is valid, the server accepts the request.
...
2. Access your application with an access token, the following claims in access token will be validated:
  ...
  - `aud`: If `spring.cloud.azure.active-directory.credential.client-id` or `spring.cloud.azure.active-directory.credential.app-id-uri` configured, the audience must be equal to the configured `client-id` or `app-id-uri`. If these 2 properties are not configured, this claim will not be validated.
...
When accessing `/scope1` endpoint, the following claims in access token will be validated:
...
When accessing `/app-role1` endpoint, the following claims in the `access token` will be validated:
...
Set property `spring.cloud.azure.active-directory.application-type` to `web_application_and_resource_server`, and specify the `authorization grant type` for each authorization client.
...
Configure multiple `HttpSecurity` instances, `AadWebApplicationAndResourceServerConfig` contains two security configurations for `resource server` and `web application`.
...
The application type can be inferred from the dependencies: spring-security-oauth2-client or spring-security-oauth2-resource-server. If the inferred value is not what you expect, you can explicitly specify it. Here is the table describing the valid values and the corresponding inferred value:
