...
Now you start the application and access your application by a browser, then you will be redirected to the Microsoft login page.
...
2. Access your application with an access token, the following claims in access token will be validated:
  ...
  - `aud`: If `spring.cloud.azure.active-directory.credential.client-id` or `spring.cloud.azure.active-directory.credential.app-id-uri` configured, the audience must be equal to the configured `client-id` or `app-id-uri`. If this 2 properties are not configured, this claim will not be validated.
