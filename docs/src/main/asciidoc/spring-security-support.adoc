 ...
 *Application ID* as your `${WEB_APP_CLIENT_ID}` and then click *Save*.

. Select *Keys* from your application, click *Generate key* to generate `${WEB_APP_CLIENT_SECRET}` and then *Save*.
...
. Specify your application *Name*, we call it `webApiA`, record the *Application ID* as your `${WEB_API_A_CLIENT_ID}` and then click *Save*.

. Select *Keys* from your application, click *Generate key* to generate `${WEB_API_A_CLIENT_SECRET}` and then *Save*.
    ...
    azure:
      active-directory:
        b2c:
          ...
          client-id: ${CLIENT_ID}           # If you are using v2.0 token, please configure client-id for `aud` verification
----
