...
From version 4.4.0, Spring Cloud Azure supports various types of credentials for authentication to `Azure Database for PostgreSQL single server`.
...
image:https://review.learn.microsoft.com/en-us/azure/developer/java/spring-framework/media/spring-cloud-azure/authentication-postgresql-azure-active-directory.png[Authentication PostgreSQL Azure Active Directory]
...
NOTE: Remember to add the BOM `spring-cloud-azure-dependencies` along with the above dependency. For details, see the link:index.html#getting-started[Getting started] section.

=== Basic usage

The following sections show the classic Spring Boot application usage scenarios.

IMPORTANT: Passwordless connection using Azure AD authentication. To use Azure AD authentication, we should set the Azure AD admin user first. Only an Azure AD Admin user can create/enable users for Azure AD-based authentication. Before starting to use the following usage scenarios, you need link:https://learn.microsoft.com/azure/developer/java/spring-framework/configure-spring-data-jdbc-with-azure-postgresql?branch=release-cred-free-java&tabs=passwordless#create-a-postgresql-server-and-set-up-admin-user[create a PostgreSQL server and set up admin user].

==== Connect to Azure PostgreSQL locally without password

1. link:https://learn.microsoft.com/azure/developer/java/spring-framework/configure-spring-data-jdbc-with-azure-postgresql?branch=release-cred-free-java&tabs=passwordless#create-a-postgresql-non-admin-user-and-grant-permission[Create a PostgreSQL non-admin user and grant permission]
2. configure the following properties in `application.yml`:
...
+
Save the sql script of creating an Azure AD service principal user to local with the following command:
+
...
----
+
Run the sql script to create the Azure AD service principal user:
+
[source,bash]
----
...
1. link:/azure/developer/java/spring-framework/migrate-postgresql-to-passwordless-connection?branch=release-cred-free-java&tabs=sign-in-azure-cli%2Cjava%2Cazure-portal%2Cspring-apps%2Cspring-apps-identity#create-the-managed-identity-using-the-azure-portal[Create the managed identity using the Azure Portal]

2. link:/azure/developer/java/spring-framework/migrate-postgresql-to-passwordless-connection?branch=release-cred-free-java&tabs=sign-in-azure-cli%2Cjava%2Cazure-portal%2Cspring-apps%2Cspring-apps-identity#assign-roles-to-the-managed-identity[Assign role to managed identity]
...
NOTE: Get more information about [Deploying Spring Application Connected to Azure Database Passwordlessly to Azure Spring Apps](/azure/developer/java/spring-framework/deploy-passwordless-spring-database-app?branch=pr-en-us-3511&tabs=postgresql)

include::migrate-postgresql-to-passwordless-connection.adoc[]
