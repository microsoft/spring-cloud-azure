...
Feature Variants are multiple instance of a object that differ in property value. A Feature Variant is define by four values: name, default, configuration-reference, and assignment-parameters. The Feature Variant name is an identifier for the variant. Default identifies which of the variants to return when no valid match is found. Configuration-reference is a pointer to which `@ConfigurationProperties` are used for that variant. Assignment-parameters are the set of properties used by an assigner to say when this variant should/shouldn't be used.
