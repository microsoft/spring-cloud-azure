...
NOTE: As of Spring Cloud Azure AutoConfigure 4.1.0, a `ThreadPoolTaskExecutor` bean named **springCloudAzureCredentialTaskExecutor** will be automatically registered by default, which will manage all threads created by Azure Identity. Each thread name managed by this thread pool is prefixed with **az-identity-**. This `ThreadPoolTaskExecutor` bean is independent of the `Executor` bean provided by Spring Boot.
