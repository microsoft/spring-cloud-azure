...
be assigned a role of `Contributor` of the associated namespace at least. Please refer to link:reference-doc#authorize-access-with-azure-active-directory[Authorize access with Azure AD] to make sure the principal has been granted the sufficient permission to access the Azure resource.
