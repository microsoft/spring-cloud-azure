...
NOTE: From version 4.0.0, when the property of **spring.cloud.azure.eventhubs.processor.checkpointStore.createContainerIfNotExists** is not enabled manually, no Storage container will be created automatically with the name from **spring.cloud.azure.eventhubs.eventHubName**.
