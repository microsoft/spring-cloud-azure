...
These properties are exposed via `com.azure.spring.cloud.autoconfigure.implementation.eventhubs.properties.AzureEventHubsProperties.Processor.BlobCheckpointStore`, which is the default implementation of `com.azure.messaging.eventhubs.CheckpointStore` to use Storage Blobs for persisting partition ownership and checkpoint information.
